...
//    primitives::{Diversifier, Note, PaymentAddress, ProofGenerationKey, Rseed, ViewingKey, SaplingIvk},
...
// ChileBob's attempt to be clever
...
pub extern "C" fn librustzcash_xfvk_decrypt_note ( 
    ...
    cmu.reverse();                                              // reverse bytes

    let mut epk = unsafe {                                      // safely read ephemeral key
        (&*raw_epk).to_vec()                                    //
    };                                                          //
    epk.reverse();                                              // reverse bytes
    ...
    let cmu = output.cmu().unwrap();                            // unwrap decoded outputs
    let epk = output.epk().unwrap();

    match try_sapling_note_decryption (                         // now attempt to decode the output
        &MainNetwork, 
        BlockHeight::from_u32(raw_height),               
        &ivk,
        &epk,
        &cmu,
        ciphertext,
    ) {
        Some((note, _to, memo)) => {                            // success !! 

            let ptr_val = unsafe { &mut *_ret_value };          // return note value to FFI
             *ptr_val = note.value.to_be_bytes();

            let ptr_memo = unsafe { &mut *_ret_memo };          // return the memo to FFI
             *ptr_memo = *memo.as_array();

            1                                                   // fn() return value to flag success
        }
        None => {                                               // failed, not our key so not our note
            0
        }
    }
}
