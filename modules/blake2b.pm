...
# Source : https://git.hush.is/hush/hushwebminer/src/branch/master/pool-emu/blake2b.pm
...
sub new {
	...
	$h->{Sh} = [ unpack 'Q<*', pack ('Q<*', @IV) ^ pack ('C C C C @32 a16 a16', $h->{hashlen}, length $h->{key}, 1, 1, $h->{salt}, $h->{personal}) ];
...
for (sort keys %test) {
	...
	$x eq pack 'H*', $test{$_} or die "$_: ${\unpack 'H*', $x} != $test{$_}";
}
